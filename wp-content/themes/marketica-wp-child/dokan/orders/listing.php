                    ...
                    <td class="dokan-order-action" width="17%">
                        ...
                        if ( dokan_get_option( 'order_status_change', 'dokan_selling', 'on' ) == 'on' ) {
                            ...
                            if ( in_array( $the_order->post_status, array('wc-pending', 'wc-on-hold') ) )
                                $confirm_actions['processing'] = array(
                            ...
                            if ( in_array( $the_order->post_status, array('wc-pending', 'wc-on-hold', 'wc-processing') ) )
                                $confirm_actions['complete'] = array(
                        ...
                        foreach ($confirm_actions as $action) {
                            $icon = ( isset( $action['icon'] ) ) ? $action['icon'] : '';
                            printf( '<button class="dokan-btn dokan-btn-default dokan-btn-sm tips" data-href="%s" data-toggle="modal" data-target="#confirmModal" rel="tooltip" data-placement="top" title="%s" data-title="%s">%s</a> ', esc_url( $action['url'] ), esc_attr( $action['name'] ), esc_attr( $action['name'] ), $icon );
                        }
