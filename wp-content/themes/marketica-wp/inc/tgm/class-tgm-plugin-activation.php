 ...
 * @version   2.4.2tokopress
 * @author    Thomas Griffin <thomasgriffinmedia.com>
 * @author    Gary Jones <gamajo.com>
 * @author    TokoPress <toko.press>
    ...
    class TGM_Plugin_Activation {
        ...
        public function init() {
            ...
            $this->strings = array(
                ...
                'installing'                     => __( 'Installing Plugin: %s', 'tokopress' ),
                'oops'                           => __( 'Something went wrong.', 'tokopress' ),
                'notice_can_install_required'    => _n_noop( 'This theme requires the following plugin: %1$s.', 'This theme requires the following plugins: %1$s.', 'tokopress' ),
                'notice_can_install_recommended' => _n_noop( 'This theme recommends the following plugin: %1$s.', 'This theme recommends the following plugins: %1$s.', 'tokopress' ),
                'notice_cannot_install'          => _n_noop( 'Sorry, but you do not have the correct permissions to install the %s plugin. Contact the administrator of this site for help on getting the plugin installed.', 'Sorry, but you do not have the correct permissions to install the %s plugins. Contact the administrator of this site for help on getting the plugins installed.', 'tokopress' ),
                'notice_can_activate_required'   => _n_noop( 'The following required plugin is currently inactive: %1$s.', 'The following required plugins are currently inactive: %1$s.', 'tokopress' ),
                'notice_can_activate_recommended'=> _n_noop( 'The following recommended plugin is currently inactive: %1$s.', 'The following recommended plugins are currently inactive: %1$s.', 'tokopress' ),
                'notice_cannot_activate'         => _n_noop( 'Sorry, but you do not have the correct permissions to activate the %s plugin. Contact the administrator of this site for help on getting the plugin activated.', 'Sorry, but you do not have the correct permissions to activate the %s plugins. Contact the administrator of this site for help on getting the plugins activated.', 'tokopress' ),
                'notice_ask_to_update'           => _n_noop( 'The following plugin needs to be updated to its latest version to ensure maximum compatibility with this theme: %1$s.', 'The following plugins need to be updated to their latest version to ensure maximum compatibility with this theme: %1$s.', 'tokopress' ),
                'notice_cannot_update'           => _n_noop( 'Sorry, but you do not have the correct permissions to update the %s plugin. Contact the administrator of this site for help on getting the plugin updated.', 'Sorry, but you do not have the correct permissions to update the %s plugins. Contact the administrator of this site for help on getting the plugins updated.', 'tokopress' ),
                'install_link'                   => _n_noop( 'Begin installing plugin', 'Begin installing plugins', 'tokopress' ),
                'update_link'                   => _n_noop( 'Begin updating plugin', 'Begin updating plugin', 'tokopress' ),
                'activate_link'                  => _n_noop( 'Begin activating plugin', 'Begin activating plugins', 'tokopress' ),
                'return'                         => __( 'Return to Required Plugins Installer', 'tokopress' ),
                'dashboard'                      => __( 'Return to the dashboard', 'tokopress' ),
                'plugin_activated'               => __( 'Plugin activated successfully.', 'tokopress' ),
                'activated_successfully'         => __( 'This plugin was activated successfully:', 'tokopress' ),
                'complete'                       => __( 'All plugins installed and activated successfully. %1$s', 'tokopress' ),
                'dismiss'                        => __( 'Dismiss this notice', 'tokopress' ),
            );
        ...
        public function admin_init() {
            ...
            if ( isset( $_REQUEST['tab'] ) && 'plugin-information' == $_REQUEST['tab'] ) {
                tokopress_require_file ( ABSPATH . 'wp-admin/includes/plugin-install.php' ); // Need for install_plugin_information().
        ...
        public function admin_menu() {
            ...
            foreach ( $this->plugins as $plugin ) {
                if ( ! is_plugin_active( $plugin['file_path'] ) || ( isset( $plugin['version'] ) && version_compare( $installed_plugins[$plugin['file_path']]['Version'], $plugin['version'], '<' ) ) ) {
                    add_theme_page(
        ...
        public function install_plugins_page() {
            ...
            <div class="tgmpa wrap">
                ...
                <style>
                #tgmpa-plugins .row-actions { visibility: visible; }
                </style>
        ...
        protected function do_plugin_install() {
            ...
            if ( isset( $_GET[sanitize_key( 'plugin' )] ) && ( ( isset( $_GET[sanitize_key( 'tgmpa-install' )] ) && 'install-plugin' == $_GET[sanitize_key( 'tgmpa-install' )] ) || ( isset( $_GET[sanitize_key( 'tgmpa-update' )] ) && 'update-plugin' == $_GET[sanitize_key( 'tgmpa-update' )] ) ) ) {
                ...
                tokopress_require_file ( ABSPATH . 'wp-admin/includes/plugin-install.php' ); // Need for plugins_api.
                tokopress_require_file ( ABSPATH . 'wp-admin/includes/class-wp-upgrader.php' ); // Need for upgrade classes.
                ...
                $url   = esc_url_raw( add_query_arg( array( 'action' => 'install-plugin', 'plugin' => $plugin['slug'] ), 'update.php' ) );
                if ( isset( $_GET['from'] ) ) {
                    $url .= esc_url_raw( add_query_arg( 'from', urlencode( stripslashes( $_GET['from'] ) ), $url ) );
                }
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . admin_url() . '" title="' . __( 'Return to the Dashboard', 'tokopress' ) . '">' . __( 'Return to the Dashboard', 'tokopress' ) . '</a>' ) . '</p>';
        ...
        public function notices() {
            ...
            // Canceled: Return early if the nag message has been dismissed.
            // We still need to check if there are plugins that needs to be updated 
            // if ( get_user_meta( get_current_user_id(), 'tgmpa_dismissed_notice_' . $this->id, true ) ) {
            //     return;
            // }
            $dismissed = get_user_meta( get_current_user_id(), 'tgmpa_dismissed_notice_' . $this->id, true );
            ...
            if ( ! empty( $message ) ) {
                ...
                foreach ( $message as $type => $plugin_groups ) {
                    ...
                    $rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[$type], $count, 'tokopress' ), $imploded, $count ) . '</p>';
                }

                // Setup variables to determine if action links are needed.
                $show_install_link  = $install_link ? '<a href="' . esc_url( add_query_arg( 'page', $this->menu, admin_url( 'themes.php' ) ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'tokopress' ) . '</a>' : '';
                $show_activate_link = $activate_link ? '<a href="' . esc_url( add_query_arg( 'page', $this->menu, admin_url( 'themes.php' ) ) ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'tokopress' ) . '</a>'  : '';
                $show_update_link  = $update_link ? '<a href="' . esc_url( add_query_arg( 'page', $this->menu, admin_url( 'themes.php' ) ) ) . '">' . translate_nooped_plural( $this->strings['update_link'], $update_link_count, 'artless' ) . '</a>' : '';
                ...
                $action_links = apply_filters(
                    ...
                    array(
                        ...
                        'dismiss'  => $this->dismissable && !$dismissed ? '<a class="dismiss-notice" href="' . esc_url( add_query_arg( 'tgmpa-dismiss', 'dismiss_admin_notices' ) ) . '" target="_parent">' . $this->strings['dismiss'] . '</a>' : '',
                    )
                );
...
if ( ! class_exists( 'WP_List_Table' ) ) {
    tokopress_require_file ( ABSPATH . 'wp-admin/includes/class-wp-list-table.php' );
}
    ...
    class TGMPA_List_Table extends WP_List_Table {
        ...
        protected function _gather_plugin_data() {
            ...
            foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
                if ( is_plugin_active( $plugin['file_path'] ) && ! isset( $plugin['version'] ) )  {
                    continue; // No need to display plugins if they are installed and activated & no minimum version defined.
                }

                if ( is_plugin_active( $plugin['file_path'] ) && isset( $plugin['version'] ) && version_compare( $installed_plugins[$plugin['file_path']]['Version'], $plugin['version'], '>=' ) )  {
                    continue; // No need to display plugins if they are installed and activated and up to date.
                }
                ...
                if ( ! empty( $plugin['source'] ) ) {
                    ...
                    if ( preg_match( '|^http(s)?://|', $plugin['source'] ) ) {
                        $table_data[$i]['source'] = __( 'Private Repository', 'tokopress' );
                        // The plugin is pre-packaged with the theme.
                    } else {
                        $table_data[$i]['source'] = __( 'Pre-Packaged', 'tokopress' );
                    }
                }
                // The plugin is from the WordPress repository.
                else {
                    $table_data[$i]['source'] = __( 'WordPress Repository', 'tokopress' );
                }

                $table_data[$i]['type'] = isset( $plugin['required'] ) && $plugin['required'] ? __( 'Required', 'tokopress' ) : __( 'Recommended', 'tokopress' );

                if ( ! isset( $installed_plugins[$plugin['file_path']] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Installed', 'tokopress' ) );
                } elseif ( is_plugin_inactive( $plugin['file_path'] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'tokopress' ) );
                } elseif ( ( isset( $plugin['version'] ) && version_compare( $installed_plugins[$plugin['file_path']]['Version'], $plugin['version'], '<' ) ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Updated', 'tokopress' ) );
                } else {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Active', 'tokopress' ) );
                }
        ...
        public function column_plugin( $item ) {
            ...
            if ( ! isset( $installed_plugins[$item['file_path']] ) ) {
                $actions = array(
                    'install' => sprintf(
                        '<a href="%1$s" title="' . __( 'Install', 'tokopress' ) . ' %2$s">' . __( 'Install', 'tokopress' ) . '</a>',
                        esc_url( 
                            wp_nonce_url(
                                add_query_arg(
                                    array(
            ...
            elseif ( is_plugin_inactive( $item['file_path'] ) ) {
                $actions = array(
                    'activate' => sprintf(
                        '<a href="%1$s" title="' . __( 'Activate', 'tokopress' ) . ' %2$s">' . __( 'Activate', 'tokopress' ) . '</a>',
                        esc_url(
                            add_query_arg(
                                array(
                                    'page'                 => TGM_Plugin_Activation::$instance->menu,
        ...
        public function column_cb( $item ) {
            ...
            $value = $item['file_path'] . ',' . $item['url'] . ',' . $item['sanitized_plugin'] . ',' . $item['status'];
        ...
        public function no_items() {
            ...
            printf( __( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', 'tokopress' ), admin_url() );
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
        ...
        public function get_columns() {
            $columns = array(
                ...
                'plugin' => __( 'Plugin', 'tokopress' ),
                'source' => __( 'Source', 'tokopress' ),
                'type'   => __( 'Type', 'tokopress' ),
                'status' => __( 'Status', 'tokopress' )
            );
        ...
        public function get_bulk_actions() {
            ...
            $actions = array(
                'tgmpa-bulk-install'  => __( 'Install', 'tokopress' ),
                'tgmpa-bulk-activate' => __( 'Activate', 'tokopress' ),
                'tgmpa-bulk-update'  => __( 'Update', 'tokopress' ),
            );
        ...
        public function process_bulk_actions() {
            ...
            if ( 'tgmpa-bulk-install' === $this->current_action()
                ...
                foreach ( $plugin_installs as $key => $plugin ) {
                    if ( ( preg_match( '|.php$|', $plugin ) && 'tgmpa-bulk-install' == $this->current_action() ) ||  ( ! preg_match( '|.php$|', $plugin ) && 'tgmpa-bulk-update' == $this->current_action() ) ) {
                ...
                if ( empty( $plugin_installs ) && 'tgmpa-bulk-install' == $this->current_action() ) {
                    echo '<div id="message" class="error"><p>' . __( 'No plugins are available to be installed at this time.', 'tokopress' ) . '</p></div>';
                    return false;
                }
                // No need to proceed further if we have no plugins to update.
                elseif ( empty( $plugin_installs ) && 'tgmpa-bulk-update' == $this->current_action() ) {
                    echo '<div id="message" class="error"><p>' . __( 'No plugins are available to be updated at this time.', 'tokopress' ) . '</p></div>';
                    return false;
                }
                ...
                $url   = esc_url_raw( add_query_arg( array( 'page' => TGM_Plugin_Activation::$instance->menu ), admin_url( 'themes.php' ) ) );
                $nonce = 'bulk-plugins';
                $names = $plugin_names;
            ...
            if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
                ...
                if ( empty( $plugins_to_activate ) ) {
                    echo '<div id="message" class="error"><p>' . __( 'No plugins are available to be activated at this time.', 'tokopress' ) . '</p></div>';
                    return false;
                }
                ...
                if ( is_wp_error( $activate ) ) {
                    ...
                    printf( '<div id="message" class="updated"><p>%1$s %2$s</p></div>', _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'tokopress' ), $imploded );
                }
...
if ( ! function_exists( 'tgmpa_load_bulk_installer' ) ) {
    function tgmpa_load_bulk_installer() {
        if ( ! class_exists( 'WP_Upgrader' ) && ( isset( $_GET['page'] ) && TGM_Plugin_Activation::$instance->menu === $_GET['page'] ) ) {
            tokopress_require_file ( ABSPATH . 'wp-admin/includes/class-wp-upgrader.php' );
            ...
            if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
                ...
                class TGM_Bulk_Installer extends WP_Upgrader {
                    ...
                    public function install_strings() {
                        ...
                        $this->strings['no_package']          = __( 'Install package not available.', 'tokopress' );
                        $this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', 'tokopress' );
                        $this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', 'tokopress' );
                        $this->strings['installing_package']  = __( 'Installing the plugin&#8230;', 'tokopress' );
                        $this->strings['process_failed']      = __( 'Plugin install failed.', 'tokopress' );
                        $this->strings['process_success']     = __( 'Plugin installed successfully.', 'tokopress' );
                    ...
                    public function activate_strings() {
                        ...
                        $this->strings['activation_failed']  = __( 'Plugin activation failed.', 'tokopress' );
                        $this->strings['activation_success'] = __( 'Plugin activated successfully.', 'tokopress' );
            ...
            if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
                ...
                class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
                    ...
                    public function add_strings() {
                        ...
                        if ( TGM_Plugin_Activation::$instance->is_automatic ) {
                            $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'tokopress' );
                            $this->upgrader->strings['skin_update_failed_error']  = __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', 'tokopress' );
                            $this->upgrader->strings['skin_update_failed']        = __( 'The installation of %1$s failed.', 'tokopress' );
                            $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', 'tokopress' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'tokopress' ) . '</span><span class="hidden">' . __( 'Hide Details', 'tokopress' ) . '</span>.</a>';
                            $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations and activations have been completed.', 'tokopress' );
                            $this->upgrader->strings['skin_before_update_header'] = __( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', 'tokopress' );
                        }
                        // Default installation strings.
                        else {
                            $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', 'tokopress' );
                            $this->upgrader->strings['skin_update_failed_error']  = __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', 'tokopress' );
                            $this->upgrader->strings['skin_update_failed']        = __( 'The installation of %1$s failed.', 'tokopress' );
                            $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed successfully.', 'tokopress' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'tokopress' ) . '</span><span class="hidden">' . __( 'Hide Details', 'tokopress' ) . '</span>.</a>';
                            $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations have been completed.', 'tokopress' );
                            $this->upgrader->strings['skin_before_update_header'] = __( 'Installing Plugin %1$s (%2$d/%3$d)', 'tokopress' );
                        }
                    ...
                    public function bulk_footer() {
                        ...
                        foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
                            if ( ! is_plugin_active( $plugin['file_path'] ) ) {
                                echo '<p><a href="' . esc_url( add_query_arg( 'page', TGM_Plugin_Activation::$instance->menu, admin_url( 'themes.php' ) ) ) . '" title="' . esc_attr( TGM_Plugin_Activation::$instance->strings['return'] ) . '" target="_parent">' . TGM_Plugin_Activation::$instance->strings['return'] . '</a></p>';
                                $complete[] = $plugin;
                                break;
                            }
                            // Nothing to store.
                        ...
                        if ( empty( $complete ) ) {
                            echo '<p>' .  sprintf( TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . admin_url() . '" title="' . __( 'Return to the Dashboard', 'tokopress' ) . '">' . __( 'Return to the Dashboard', 'tokopress' ) . '</a>' ) . '</p>';
                            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
                        }
