...
$post_types = array_merge(array('all', 'post', 'page'), get_post_types( $args, $output, $operator ));
...
<script>
    jQuery(document).ready(function($){
        $("#p_asp_submit").on('click', function(e){
            e.preventDefault();
            var data = {
                action: 'ajaxsearchpro_priorities_get_posts',
                options: $("#asp_priorities").serialize()
            };
            $('#p_asp_loader').css('display', 'block');
            $('#p_asp_results').fadeOut(10);
            var post = $.post(ajaxsearchpro.backend_ajaxurl, data, function (response) {
        ...
        $('#p_asp_results').on('click', '.p_asp_save', function(e){
            ...
            var $this = $(this);
            var data = {
                action: 'ajaxsearchpro_priorities_set_priorities',
